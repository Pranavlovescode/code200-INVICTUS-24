...
class RecommendCourses:
  def __init__(self):
    ...
    datasci_df = pd.read_csv(dirname+'/dataset/udemy.csv',encoding='latin-1')
    datasci_df.rename(columns={'name': 'course_title'}, inplace=True)
    
    df = pd.concat([business_df, design_df,music_df,web_df,entry_df,datasci_df])
...
def recommend_post():
    ...
    if data is None:
        ...
        return recommendation.recommend_course(data["query"],20) 
